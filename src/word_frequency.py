...
word_frequency_data.drop(
    word_frequency_data[word_frequency_data["word"].map(len) < 5].index, inplace=True
)
word_frequency_data.drop(
    word_frequency_data[word_frequency_data["word"].map(len) > 5].index, inplace=True
)
...
def calculate_word_frequency(rep):
    """Determine frequency heuristic value for given word

    Inputs:
        rep (str): string representation of word

    Outputs:
        int: word frequency value
    """
    try:
        count = int(
            word_frequency_data.loc[word_frequency_data["word"] == rep, "count"]
        )
    except:
